...
var testFiles = ['./test/mockContext.js', './test/*.js', '!./test/core.layoutService.tests.js', '!./test/defaultConfig.tests.js'];
...
function bumpTask(complete) {
  ...
  inquirer.prompt({
    ...
    gulp.src(['./README.md']).pipe(replace(oldCDN, newCDN)).pipe(gulp.dest('./'));
    complete();
  });
}
...
function jshintTask() {
  return gulp.src(srcDir + '**/*.js').pipe(jshint('config.jshintrc')).pipe(jshint.reporter('jshint-stylish')).pipe(jshint.reporter('fail'));
}

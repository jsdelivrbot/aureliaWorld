...
(function(process) {
  (function(root, factory) {
      ...
      beforeSetTickPositions: function() {
        ...
        if (isOrdinal || hasBreaks) {
          ...
          if (useOrdinal) {
            ...
            maxIndex = mathMax(axis.val2lin(mathMin(max, ordinalPositions[ordinalPositions.length - 1]), true), 1);
      ...
      if (gapSize && i > 0) {
        while (i--) {
          if (points[i + 1].x - points[i].x > this.closestPointRange * gapSize) {
            points.splice(i + 1, 0, {isNull: true});
          }
        }
      }
      ...
      drawPoints: function() {
        ...
        each(points, function(point) {
          if (point.plotY !== UNDEFINED) {
            ...
            if (graphic) {
              graphic.attr(pointAttr).animate({d: path});
            } else {
      ...
      drawPoints: function() {
        ...
        each(points, function(point) {
          ...
          if (point.plotY !== UNDEFINED) {
            ...
            path = [];
            path.push('M', crispX - halfWidth, bottomBox, 'L', crispX - halfWidth, topBox, 'L', crispX + halfWidth, topBox, 'L', crispX + halfWidth, bottomBox, 'Z', 'M', crispX, topBox, 'L', crispX, hasTopWhisker ? mathRound(point.plotY) : topBox, 'M', crispX, bottomBox, 'L', crispX, hasBottomWhisker ? mathRound(point.yBottom) : bottomBox);
            if (graphic) {
              graphic.attr(pointAttr).animate({d: path});
            } else {
    ...
    seriesTypes.flags = extendClass(seriesTypes.column, {
              ...
              if (point.x <= lastX) {
                ...
                if (leftPoint.x < point.x && !step) {
                  ...
                  if (rightPoint && rightPoint[onKey] !== UNDEFINED) {
                    point.plotY += ((point.x - leftPoint.x) / (rightPoint.x - leftPoint.x)) * (rightPoint[onKey] - leftPoint[onKey]);
                  }
                }
              }
              cursor--;
      ...
      drawPoints: function() {
        ...
        while (i--) {
          ...
          plotX = point.plotX;
          if (plotX > 0) {
            plotX -= pick(point.lineWidth, options.lineWidth) % 2;
          }
    ...
    symbols.flag = function(x, y, w, h, options) {
      ...
      return ['M', anchorX, anchorY, 'L', x, y + h, x, y, x + w, y, x + w, y + h, x, y + h, 'Z'];
    };
    ...
    var units = [].concat(defaultDataGroupingUnits),
        defaultSeriesType,
    ...
    Scroller.prototype = {
      drawHandle: function(x, index) {
        ...
        if (!scroller.rendered) {
          handles[index] = renderer.g('navigator-handle-' + ['left', 'right'][index]).css({cursor: 'ew-resize'}).attr({zIndex: 10 - index}).add();
      ...
      drawScrollbarButton: function(index) {
        ...
        if (!scroller.rendered) {
          ...
          tempElem = renderer.rect(-0.5, -0.5, scrollbarHeight + 1, scrollbarHeight + 1, scrollbarOptions.buttonBorderRadius, scrollbarOptions.buttonBorderWidth).attr({
        ...
        if (index) {
          scrollbarButtons[index].attr({translateX: scroller.scrollerWidth - scrollbarHeight});
        }
      },
      render: function(min, max, pxMin, pxMax) {
        ...
        if (!isNumber(pxMin) || mathAbs(pxMin) === Infinity) {
          pxMin = 0;
          pxMax = scrollerWidth;
        }
        ...
        if (!rendered) {
          if (navigatorEnabled) {
            scroller.navigatorGroup = navigatorGroup = renderer.g('navigator').attr({zIndex: 3}).add();
            ...
            if (navigatorOptions.maskInside) {
              ...
              scroller.rightShade = renderer.rect().attr({fill: navigatorOptions.maskFill}).add(navigatorGroup);
            }
      ...
      addEvents: function() {
        var chart = this.chart,
            ...
            _events;
        _events = [[container, 'mousedown', mouseDownHandler], [container, 'mousemove', mouseMoveHandler], [doc, 'mouseup', mouseUpHandler]];
        if (hasTouch) {
          _events.push([container, 'touchstart', mouseDownHandler], [container, 'touchmove', mouseMoveHandler], [doc, 'touchend', mouseUpHandler]);
        }
      ...
      init: function() {
        ...
        scroller.mouseDownHandler = function(e) {
          ...
          var zoomedMin = scroller.zoomedMin,
              ...
              left,
              isOnNavigator;
          if (chartY > top && chartY < top + height + scrollbarHeight) {
            ...
            if (isOnNavigator && math.abs(chartX - zoomedMin - navigatorLeft) < handleSensitivity) {
              ...
              if (isOnNavigator) {
                ...
                if (chartX < navigatorLeft) {
                  ...
                  left = chartX < navigatorLeft + zoomedMin ? zoomedMin - range : zoomedMax;
                }
              }
              if (left < 0) {
                left = 0;
        ...
        scroller.mouseUpHandler = function(e) {
          ...
          if (e.type !== 'mousemove') {
            scroller.grabbedLeft = scroller.grabbedRight = scroller.grabbedCenter = scroller.fixedWidth = scroller.fixedExtreme = scroller.otherHandlePos = scroller.hasDragged = dragOffset = null;
          }
        };
    ...
    RangeSelector.prototype = {
      clickButton: function(i, redraw) {
        ...
        if (dataMin === null || dataMax === null || i === rangeSelector.selected) {
          return;
        }
      ...
      updateButtonStates: function(updating) {
        ...
        each(rangeSelector.buttonOptions, function(rangeOptions, i) {
          var actualRange = mathRound(baseAxis.max - baseAxis.min),
              ...
              isAllButAlreadyShowingAll = rangeOptions.type === 'all' && baseAxis.max - baseAxis.min >= dataMax - dataMin && buttons[i].state !== 2,
          ...
          if ((type === 'month' || type === 'year') && (actualRange >= {
            month: 28,
            year: 365
          }[type] * 24 * 36e5 * count) && (actualRange <= {
            month: 31,
            year: 366
          }[type] * 24 * 36e5 * count)) {
            isSameRange = true;
        ...
        function updateExtremes() {
          ...
          if (value !== input.previousValue) {
            ...
            if (isNumber(value)) {
              ...
              if (value !== UNDEFINED) {
                chart.xAxis[0].setExtremes(isMin ? value : xAxis.min, isMin ? xAxis.max : value, UNDEFINED, UNDEFINED, {trigger: 'rangeSelectorInput'});
              }
            }
          }
        }
      ...
      render: function(min, max) {
        ...
        if (!rendered) {
          ...
          rangeSelector.zoomText = renderer.text(lang.rangeSelectorZoom, pick(buttonPosition.x, plotLeft), 15).css(options.labelStyle).add(buttonGroup);
          ...
          each(rangeSelector.buttonOptions, function(rangeOptions, i) {
            ...
            }, buttonTheme, states && states.hover, states && states.select, states && states.disabled).css({textAlign: 'center'}).add(buttonGroup);
        ...
        buttonGroup[rendered ? 'animate' : 'attr']({translateY: pos.buttonTop});
        if (inputEnabled !== false) {
          inputGroup.align(extend({
            y: pos.inputTop,
            width: inputGroup.offset,
            x: navButtonOptions && (pos.inputTop < (navButtonOptions.y || 0) + navButtonOptions.height - chart.spacing[0]) ? -40 : 0
    ...
    wrap(Axis.prototype, 'drawCrosshair', function(proceed, e, point) {
      ...
      if (!crossLabel) {
        crossLabel = this.crossLabel = chart.renderer.label(null, null, null, options.shape || 'callout').attr({
    ...
    seriesProto.setCompare = function(compare) {
      this.modifyValue = (compare === 'value' || compare === 'percent') ? function(value, point) {
        ...
        if (value !== UNDEFINED) {
          value = compare === 'value' ? value - compareValue : value = 100 * (value / compareValue) - 100;
          if (point) {
            point.change = value;
          }
        }
    ...
    seriesProto.processData = function() {
      ...
      if (series.xAxis && series.processedYData) {
        ...
        for (i = 0; i < length; i++) {
          compareValue = keyIndex > -1 ? processedYData[i][keyIndex] : processedYData[i];
    ...
    Point.prototype.tooltipFormatter = function(pointFormat) {
      var point = this;
      pointFormat = pointFormat.replace('{point.change}', (point.change > 0 ? '+' : '') + Highcharts.numberFormat(point.change, pick(point.series.tooltipOptions.changeDecimals, 2)));
    ...
    wrap(Series.prototype, 'render', function(proceed) {
      if (this.chart.options._stock && this.xAxis) {
        if (!this.clipBox && this.animate) {
          ...
          stop(this.chart[this.sharedClipKey]);
